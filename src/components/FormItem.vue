  ...
  methods: {
    ...
    handleValueChange(val) {
      ...
      if (this.field.type === 'daterange' || this.field.type === 'datetimerange') {
        if (v[0] && v[1]) {
          v = [
            Helper.formatDate(v[0], this.field.options.format, this.field.dataType),
            Helper.formatDate(v[1], this.field.options.format, this.field.dataType)
          ]
        }
    ...
    handleSwitchValueChange(isChecked) {
      this.handleValueChange(isChecked ? this.field.options.on.value : this.field.options.off.value)
    ...
    renderDate(h) {
      ...
      if (this.field.options && this.field.options['disabledDate'] && typeof this.field.options['disabledDate'] == 'string') {
        pickerOptions.disabledDate = this.getDatePickerDisableDate(this.field.options['disabledDate'], this.field.options['disabledDateFilter'])
    ...
    renderDateRange(h) {
      ...
      if (this.field.options && this.field.options.shortcuts) {
        ...
        pickerOptions.shortcuts = this.field.options.shortcuts.map(shortcut => ({
  ...
  render(h) {
    ...
    switch (this.field.type) {
      ...
      case 'enum':
        ele = (
          <span>
            {this.field.options.maps[this.field.value]}
          </span>
        )
        break
      case 'image':
        ele = (
          <image-viewer src={this.field.value}>
          </image-viewer>
        )
        break
      case 'text-list':
        ele = (
          <ul class="text-list">
            {(this.field.value || []).map(item => <li>{item}</li>)}
          </ul>
        )
        break
      case 'text':
