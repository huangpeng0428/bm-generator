...
export default {
  ...
  data() {
    return {
      ...
      daterangeTypes: [{ label: '无', value: '' }].concat(Helper.convertMapToArray(Config.daterangeShortcuts)),
  ...
  computed: {
    ...
    supportButtonTypes() {
      return this.buttonTypes.filter((button) => this.buttonDialog.supportTypes.indexOf(button.value) !== -1);
    },
  },
  ...
  methods: {
            ...
            this.$set(
              ...
              Helper.assign(
                ...
                fieldEntity.type == 'table-column-hidden'
                  ? { data: Helper.assign({}, this.fieldTableColumnHiddenGroupData) }
                  : {},
              ),
            );
          },
    ...
    handleButtonDialogOpen(buttonInfo, save, origin) {
      ...
      if (buttonInfo !== null) {
        ...
        this.watchPropertyComplete(this.buttonEntity, [{ name: 'type', value: buttonEntity.type }], () => {
      ...
      switch (origin) {
        case 'searchForm':
          ...
          this.buttonDialog.supportTypes = ['search', 'export', 'import', 'link', 'router', 'dialog', 'reset'];
          break;
        case 'dataTable':
          if (isAdd) this.buttonEntity.type = 'dialog';
          this.buttonDialog.supportTypes = ['link', 'router', 'dialog', 'confirm'];
          break;
        case 'dialog':
          if (isAdd) this.buttonEntity.type = 'request';
          this.buttonDialog.supportTypes = ['close', 'request', 'dialog', 'confirm'];
          break;
        default:
          break;
      }
    ...
    handleColumnDialogOpen(columnInfo, save) {
      if (columnInfo !== null) {
        ...
        this.watchPropertyComplete(this.columnEntity, [{ name: 'type', value: columnEntity.type }], () => {
    ...
    handleDialogsItemEdit(row, isCopy) {
      ...
      this.dialogItemDialog.isSameTitle =
        ...
        (this.dialogEntity.title === '' &&
          (this.dialogEntity.titles.add || this.dialogEntity.titles.edit || this.dialogEntity.titles.view) === '');
    ...
    handleDialogItemDialogSave() {
      ...
      if (dialogInfo.type === 'default' && !this.dialogItemDialog.isDynamicButton && dialogInfo.buttons.length) {
        dialogInfo.buttons = [];
      }
      this.dialogItemDialog.save(dialogInfo);
