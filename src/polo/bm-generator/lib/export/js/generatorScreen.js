...
export default {
  ...
  computed: {
    ...mapState(['permissionTree', 'navChildPage']),
      ...
      this.permissionTree.forEach((tree) => {
        if (tree.children.length == 0) {
          ...
          tree.children.forEach((item) => {
            ...
            } else if (item.url === this.navChildPage[url]) {
              // 在第三层级中
    ...
    hideSearchForm() {
      return (
        this.searchForm.buttons.length == 0 && this.searchForm.fields.filter((p) => p.type != 'hidden').length == 0
      );
    },
    ...
    searchFormInfo() {
      ...
      this.searchForm.fields.forEach((field) => {
        let key = field.key;
        if (Array.isArray(key) || key.indexOf(',') != -1) {
          // 添加逗号分隔
    ...
    isChartCompareDay() {
      const params = this.getDataTableParams(
        this.searchForm.urls.chart.beforeFilter,
        '请求数据列表接口的处理查询条件有误，返回值必须是一个对象',
      );
  ...
  filters: {
    dialogViewFieldValue(val, entry) {
      ...
      switch (entry.type) {
        ...
        case 'datetime':
          if (val) {
            r = Helper.formatDate(val, entry.options.format);
          }
          break;
      }
      return r;
  ...
  methods: {
    ...
    handleDataTableSort(sortData) {
      ...
      this.handleButtonClick(
        'searchForm',
        { type: 'search' },
        { isRefresh: !this.dataTable.useLocalSort, needRemoteLoad: !this.dataTable.useLocalSort },
      );
    },
    ...
    getColumnValue(column, row, index, origin = 'list') {
      ...
      switch (column.type) {
        case 'index':
          if (origin === 'list') {
            r = index + (this.dataTable.pagination.currentPage - 1) * this.dataTable.pagination.pageSize + 1;
            if (row.isSummary) {
              r = '合计';
            }
          }
        ...
        case 'time':
          if (Array.isArray(column.options.field)) {
            ...
            if (t1 > 0 && t2 > 0) {
              let time1 =
                t1 == null || isNaN(Number(t1)) || (typeof t1 == 'number' && t1 < 1000 * 3600 * 24)
                  ? t1
                  : Helper.formatDate(t1, column.format);
              let time2 =
                t2 == null || isNaN(Number(t2)) || (typeof t2 == 'number' && t2 < 1000 * 3600 * 24)
                  ? t2
                  : Helper.formatDate(t2, column.format);
              r = time1 + '至' + time2;
            ...
            r =
              time == null || isNaN(Number(time)) || (typeof time == 'number' && time < 1000 * 3600 * 24)
                ? time
                : Helper.formatDate(time, column.format);
          }
          break;
        case 'date':
    ...
    handleDialogSaveButtonVisible(dialog) {
      ...
      if (dialog.status === 'disabled' || dialog.status === 'view') {
        visible = false;
      }
    ...
    handleButtonClick(origin, btn, data) {
      ...
      if (origin == 'dataTableBatch' && this.dataTableSelectedRows && this.dataTableSelectedRows.length == 0) {
        // 如果没有勾选数据
      ...
      if (origin == 'dataTableBatch') {
        ...
        if (Helper.isPureObject(batchParams)) {
          ...
          batchParams.forEach((param) => {
            if (typeof param === 'string' || (Helper.isPureObject(param) && 'value' in param)) {
      ...
      switch (btn.type) {
        ...
        case 'dialog': {
          ...
          if (btn.options.status === 'add') {
            ...
            this.handleDialog(btn.options.name, true, btn.options.status, Helper.assign({}, this.$route.query, data));
            this.updateDialogFieldsValue(btn.options.name, Helper.assign({}, this.$route.query, data));
          }
          break;
        }
        case 'confirm':
        ...
        case 'batch-match': {
          ...
          matchStatus.forEach((item, index) => {
            if (item != 2 || (appStatus[index] != 50 && appStatus[index] != 70)) {
              // 匹配有结果 并且应用状态是 上架或者下架才可以
      ...
      switch (field.type) {
        ...
        case 'datetime':
          if (!Helper.isEmptyValue(field.defaultValue)) {
            field.value = Helper.formatDate(Helper.getDate(field.defaultValue), field.options.format);
          } else {
    ...
    handleSortJump(row, type) {
      ...
      this.sendSortRequest(this.dataTable.sortUrls[type], Helper.assign({}, row, { order: this.jumpOrder }), type);
    },
    ...
    getSummaryDataTableData() {
      ...
      if (
        ...
        Object.keys(this.dataTable.summary).length == 0 ||
        Object.values(this.dataTable.summary).filter((p) => p !== null).length == 0
      ) {
        this.dataTable.summary = null;
      }
    ...
    loadDataTableList(options) {
      ...
      const params = this.getDataTableParams(
        this.searchForm.urls.list.beforeFilter,
        '请求数据列表接口的处理查询条件有误，返回值必须是一个对象',
      );
          ...
          success: (res) => {
            ...
            if (this.searchForm.urls.list.filter) {
              ...
              if (typeof this.searchForm.urls.list.filter === 'string') {
                this.searchForm.urls.list.filter = new Function(
                  'res',
                  'params',
                  'url',
                  'route',
                  this.searchForm.urls.list.filter,
                );
              }
              /* eslint-enable no-new-func */
    ...
    loadDataTableTotalData() {
      ...
      if (this.searchForm.urls.total.url || this.searchForm.urls.total.urlFilter) {
        ...
        const params = this.getDataTableParams(
          this.searchForm.urls.total.beforeFilter,
          '请求数据列表总数接口的处理查询条件有误，返回值必须是一个对象',
        );
            ...
            success: (res) => {
              ...
              if (this.searchForm.urls.total.filter) {
                ...
                if (typeof this.searchForm.urls.total.filter === 'string') {
                  this.searchForm.urls.total.filter = new Function(
                    'res',
                    'params',
                    'url',
                    this.searchForm.urls.total.filter,
                  );
                }
                /* eslint-enable no-new-func */
    ...
    loadDataTableSummaryData() {
      ...
      if (this.searchForm.urls.summary.url || this.searchForm.urls.summary.urlFilter) {
        if (
          this.searchForm.urls.summary.beforeFilter &&
          typeof this.searchForm.urls.summary.beforeFilter === 'string'
        ) {
          /* eslint-disable no-new-func */
        ...
        const params = this.getDataTableParams(
          this.searchForm.urls.summary.beforeFilter,
          '请求数据列表汇总接口的处理查询条件有误，返回值必须是一个对象',
        );
            ...
            success: (res) => {
              ...
              if (this.searchForm.urls.summary.filter) {
                ...
                if (typeof this.searchForm.urls.summary.filter === 'string') {
                  this.searchForm.urls.summary.filter = new Function(
                    'res',
                    'params',
                    'url',
                    this.searchForm.urls.summary.filter,
                  );
                }
                /* eslint-enable no-new-func */
    ...
    handleExportReport() {
      ...
      const exportFn = (result) => {
        let excelTitle =
          this.breadcrumb.map((p) => Helper.textParse(p.title, this.$route.query)).join('-') +
          this.searchFormFields
            .filter((p) => ['daterange', 'datetimerange'].indexOf(p.type) !== -1)
            .map((p) => `(${p.value.join('~')})`)
            .join('');
        ExportJS.toCSV({
    ...
    loadChartData({ isRefresh = true, needRemoteLoad = true }) {
      ...
      if (this.isChartCompareDay) {
        ...
        this.chart.compare.day.forEach((p) => {
          ...
          _params[dateKeys[1]] = _params[dateKeys[0]] = Helper.formatDate(
            Helper.getDate(p, new Date(_params[dateKeys[0]].replace(/-/g, '/'))),
          );
    ...
    loadRemoteChartData({ params = null, isRefresh = true, type }) {
      ...
      if (
        ...
        !(
          this.chart &&
          (this.searchForm.urls.chart.url || this.searchForm.urls.chart.urlFilter) &&
          this.chart.xAxisKey &&
          this.chart.xAxisKey.key
        )
      ) {
        return;
      }
      ...
      const success = ({ data }) => {
        ...
        if (type == 'today' || !type) {
          ...
          this.chart.options.xAxis = Helper.assign(
            {},
            this.chart.options.xAxis,
            JSON.parse(this.chart.xAxisKey.options || '{}'),
          );
        }
          ...
          success: (res) => {
            ...
            if (this.searchForm.urls.chart.filter) {
              ...
              if (typeof this.searchForm.urls.chart.filter === 'string') {
                this.searchForm.urls.chart.filter = new Function(
                  'res',
                  'params',
                  'url',
                  this.searchForm.urls.chart.filter,
                );
              }
              /* eslint-enable no-new-func */
      ...
      if (!type || type == 'today') {
        this.chart.options.xAxis.data = (this.chart.resultData[type] || this.chart.resultData).map((d) => {
          ...
          switch (dataType) {
            ...
            case 'date':
              value =
                value == null || isNaN(Number(value)) || (typeof value == 'number' && value < 1000 * 3600 * 24)
                  ? value
                  : Helper.formatData(value, dataType);
              break;
            default:
          }
          return value;
    ...
    renderChartLegend(type, selected) {
      if (!type || this.chart.compare.day.length > 0) {
        ...
        const day = Object.keys(Config.chartCompareDayMap).filter((p) => this.chart.compare.day.indexOf(p) != -1)[0];
        ...
        this.chart.options.legend.selected = this.chart.seriesKeys.reduce((p, c) => {
          p[`${prefix}${c.name}`] = selected
            ? !!selected[`${this.chart.options.legend.selected.__prefix || ''}${c.name}`]
            : c.defaultShow;
          return p;
        }, {});
    ...
    renderChartHourSeries(selected) {
      ...
      const curDay = Object.keys(Config.chartCompareDayMap).filter((p) => this.chart.compare.day.indexOf(p) != -1)[0];
      const prefix = curDay == 'today' ? '' : `${Config.chartCompareDayMap[curDay]}`;
      const seriesKeys = this.chart.seriesKeys.filter(
        (d) => this.chart.options.legend.data.indexOf(`${prefix}${d.name}`) != -1,
      );
    ...
    fieldChangeDoSearch(options, load = true) {
      ...
      this.dataTable.columns = (this.dataTable.initColumns || []).filter((d) => {
        return (
          !d.canHidden ||
          (d.canHidden && options && options.columnFields && options.columnFields.indexOf(d.field) != -1)
        );
      });
      if (load && !this.dataTable.loading) {
        // 需要重新加载，并且datatable未正在加载，避免重复请求
    ...
    before_setPageConfig(pageConfig, fn) {
      if (pageConfig.page.before_setPageConfig) {
        Promise.all(
          pageConfig.page.before_setPageConfig.map((config) => {
