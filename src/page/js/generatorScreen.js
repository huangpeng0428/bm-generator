...
export default {
  ...
  methods: {
    ...
    // datatable 行添加样式
    dataTableRowClassName(row, index) {
    ...
    handleDialogSaveButtonVisible(dialogName, dialog) {
      let visible = true
      ...
      switch (btn.type) {
        ...
        case 'request': {
          ...
          if (dialog.type === 'template') {
            ...
            params = btn.options.beforeFilter({
              data: dialog.data,
              dynamic: $dynamic.getDynamicValue(),
              dialog
            })
          } else {
    ...
    // sort操作column类型的回调
    handleColumnSortClick(column, row, index, type) {
      if (type == 'jump') {
        this.jumpOrder = ''
      } else {
    ...
    handleSortJump(column, row, index, type) {
    ...
    // 加载图表数据 - 精细处理
    loadRemoteChartData({ params = null, isRefresh = true, needRemoteLoad = true, type }) {
