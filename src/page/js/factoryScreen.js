...
export default {
  ...
  data() {
    return {
      ...
      namedRouter: (() => {
        const namedRouter = router.options.routes
          .map(route => ({
            label: route.name,
            value: route.name
          }))
  ...
  computed: {
    ...
    supportButtonTypes() {
      return this.buttonTypes.filter(
        button => this.buttonDialog.supportTypes.indexOf(button.value) !== -1
      );
    }
  },
  ...
  watch: {
    'fieldEntity.type'(val) {
      ...
      this.$set(
        this.fieldEntity,
        'options',
        this.getFieldEntityDefaultOptions(val, false)
      );
      switch (val) {
        case 'cascader':
        case 'autocomplete':
    ...
    'fieldEntity.isAsync'(val) {
      this.$set(
        this.fieldEntity,
        'options',
        this.getFieldEntityDefaultOptions(this.fieldEntity.type, val)
      );
    },
  ...
  methods: {
      ...
      switch (type) {
        ...
        case 'datetimerange':
          opts = {
            value: [],
            format:
              type === 'datetimerange' ? 'yyyy-MM-dd HH:mm:ss' : 'yyyy-MM-dd',
    ...
    handleFieldDataTableColumnShowEdit(row, index) {
      ...
      this.columnShowDialog.save = columnFields => {
        this.$set(
          this.fieldEntity.options.data[index],
          'columnFields',
          columnFields
        );
      };
    ...
    handleFieldAsyncConfigOpen() {
      ...
      this.urlDialog.save = urlInfo => {
        this.fieldEntity.options = Helper.assign(
          {},
          this.fieldEntity.options,
          urlInfo
        );
        ...
        this.watchPropertyComplete(
          ...
          () => {
            if (buttonEntity.type === 'router') {
              buttonEntity.options.paramsParams = JSON.stringify(
                buttonEntity.options.paramsParams
              );
              buttonEntity.options.queryParams = JSON.stringify(
                buttonEntity.options.queryParams
              );
            }
            this.buttonEntity = Helper.assign(
              {},
              defaultButtonEntity,
              buttonEntity
            );
    ...
    handleButtonDialogSave() {
      ...
      if (buttonInfo.type === 'router') {
        buttonInfo.options.paramsParams = JSON.parse(
          buttonInfo.options.paramsParams || '{}'
        );
        buttonInfo.options.queryParams = JSON.parse(
          buttonInfo.options.queryParams || '{}'
        );
      }
        ...
        this.watchPropertyComplete(
          ...
          () => {
            if (columnEntity.type === 'router') {
              columnEntity.options.queryParams = JSON.stringify(
                columnEntity.options.queryParams
              );
              columnEntity.options.paramsParams = JSON.stringify(
                columnEntity.options.paramsParams
              );
            }
            this.columnEntity = Helper.assign(
              {},
              defaultColumnEntity,
              columnEntity
            );
    ...
    handleColumnDialogSave() {
      ...
      if (columnInfo.type === 'router') {
        columnInfo.options.queryParams = JSON.parse(
          columnInfo.options.queryParams || '{}'
        );
        columnInfo.options.paramsParams = JSON.parse(
          columnInfo.options.paramsParams || '{}'
        );
      }
    ...
    handleDialogsItemEdit(row, isCopy) {
      this.dialogEntity = Helper.assign(
        {},
        defaultDialogEntity,
        row,
        isCopy ? { name: '' } : {}
      );
      this.dialogItemDialog.isSameTitle =
      ...
      if (this.dialogEntity.type === 'default') {
        this.dialogItemDialog.isDynamicButton = !!this.dialogEntity.buttons
          .length;
      }
      this.dialogItemDialog.save = dialogInfo => {
    ...
    handleChartSeriesDialogOpen(entity, save) {
      if (entity !== null) {
        this.chartSeriesEntity = Helper.assign(
          {},
          defaultChartSeriesEntity,
          entity
        );
      }
    ...
    handleUploadDialogOpen(uploadEntity, save) {
      if (uploadEntity !== null) {
        this.uploadEntity = Helper.assign(
          {},
          defaultUploadEntity,
          uploadEntity
        );
